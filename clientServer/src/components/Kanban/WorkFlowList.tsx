...
import { TicketType, ColumnProps } from "../../types";
import Ticket from "./Ticket";

const WorkFlowList: React.FC<ColumnProps> = ({ title, tickets = [], id }) => {
  const { setSelectedTicket } = useKanban();
  const [open, setOpen] = useState(false);
  const { project, setProject } = useKanban(); 

  const handleAddCard = (newCard: TicketType) => {
    if (!project) return;

    const updatedProject = { 
      ...project,
      list: project.list.map(l => 
        l._id === id ? { ...l, list: [...l.list, newCard] } : l
      )
    };
        ...
        {(provided) => (
          ...
          >
            {tickets.map((ticket, index) => (
              <Ticket index={index} ticket={ticket} key={ticket._id}  onClick={() => setSelectedTicket(ticket)} />
            ))}
